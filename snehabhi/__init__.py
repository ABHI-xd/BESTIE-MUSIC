...
snehabhi_dir = join(dirname(realpath(__file__)), ".")
...
def __all_module():
    for file in listdir(snehabhi_dir):
...
def load_module(user_id: Optional[int] = 0):
    for mods in __all_module():
        try:
            imported_mods = import_module(f"snehabhi.{mods}")
            if user_id:
                pass
            else:
                time.sleep(0.25)
                ...
                if user_id:
                    modules.append(
                        InlineKeyboardButton(
                            ...
                            callback_data=f"snehabhi.{mods.split('_')[0] if '_' in mods else mods}|{user_id}"
                        )
                    )
